package com.lanqiao.CRM.entity;

import java.util.Date;

public class Clue {
private int xid;
private String xname;
private String xcomname;
private String xdeptno;
String xjob;
String xphone;
String xemail;
String xarea;
int xzip;
String xstatus;
String xsource;
String xnext;
String xactual;
String xremark;
String xfid;
public int getXid() {
	return xid;
}
public void setXid(int xid) {
	this.xid = xid;
}
public String getXname() {
	return xname;
}
public void setXname(String xname) {
	this.xname = xname;
}
public String getXcomname() {
	return xcomname;
}
public void setXcomname(String xcomname) {
	this.xcomname = xcomname;
}
public String getXdeptno() {
	return xdeptno;
}
public void setXdeptno(String xdeptno) {
	this.xdeptno = xdeptno;
}
public String getXjob() {
	return xjob;
}
public void setXjob(String xjob) {
	this.xjob = xjob;
}
public String getXphone() {
	return xphone;
}
public void setXphone(String xphone) {
	this.xphone = xphone;
}
public String getXemail() {
	return xemail;
}
public void setXemail(String xemail) {
	this.xemail = xemail;
}
public String getXarea() {
	return xarea;
}
public void setXarea(String xarea) {
	this.xarea = xarea;
}




public int getXzip() {
	return xzip;
}
public void setXzip(int xzip) {
	this.xzip = xzip;
}
public String getXstatus() {
	return xstatus;
}
public void setXstatus(String xstatus) {
	this.xstatus = xstatus;
}
public String getXsource() {
	return xsource;
}
public void setXsource(String xsource) {
	this.xsource = xsource;
}

public String getXremark() {
	return xremark;
}
public void setXremark(String xremark) {
	this.xremark = xremark;
}

public String getXfid() {
	return xfid;
}
public void setXfid(String xfid) {
	this.xfid = xfid;
}
public String getXnext() {
	return xnext;
}
public void setXnext(String xnext) {
	this.xnext = xnext;
}
public String getXactual() {
	return xactual;
}
public void setXactual(String xactual) {
	this.xactual = xactual;
}


}
